...
class Database:
    ...
    def write_job_statuses(
        ...
        if opt_jobs:
            ...
            for job_dir, opt_job in opt_jobs.items():
                if job_dir in existing_opt_rows:
                    opt_updates.append(
                        (
                            job_dir,
                            opt_job.status.value,
                            opt_job.home_machine.value,
                            opt_job.last_on.value,
                            existing_opt_rows[job_dir],
                        )
                    )
                else:
                    opt_inserts.append(
            ...
            if opt_inserts:
                self.db.executemany(
                    "INSERT INTO opt_jobs VALUES (?, ?, ?, ?)", opt_inserts
                )
            if opt_updates:
                self.db.executemany(
        ...
        if dos_jobs:
            ...
            for job_dir, dos_job in dos_jobs.items():
                ...
                if resolved_opt_id in existing_dos_ids:
                    dos_updates.append(
                        (
                            dos_job.sc_status.value,
                            dos_job.dos_status.value,
                            dos_job.sc_last_on.value,
                            dos_job.dos_last_on.value,
                            resolved_opt_id,
                        )
                    )
                else:
            ...
            if dos_updates:
                self.db.executemany(
                    "UPDATE dos_jobs SET sc_status = ?, dos_status = ?, sc_last_on = ?, dos_last_on = ? WHERE opt_id = ?",
                    dos_updates,
                )
        ...
        if wav_jobs:
            ...
            for job_dir, wav_job in wav_jobs.items():
                ...
                if resolved_opt_id in existing_wav_ids:
                    wav_updates.append(
                        (
                            wav_job.wav_status.value,
                            wav_job.wav_last_on.value,
                            resolved_opt_id,
                        )
                    )
                else:
            ...
            if wav_inserts:
                self.db.executemany(
                    "INSERT INTO wav_jobs VALUES (?, ?, ?)", wav_inserts
                )
            if wav_updates:
                self.db.executemany(
                    "UPDATE wav_jobs SET wav_status = ?, wav_last_on = ? WHERE opt_id = ?",
                    wav_updates,
                )
