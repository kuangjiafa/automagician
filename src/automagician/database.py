...
class Database:
    ...
    def write_job_statuses(
        ...
        # Batch opt_jobs: one SELECT then executemany for inserts and updates
        opt_existing: Dict[str, int] = {
            row[1]: row[0] for row in self.db.execute("SELECT rowid, dir FROM opt_jobs")
        }
        opt_inserts = []
        opt_updates = []
        for job_dir, job in opt_jobs.items():
            row = (job_dir, job.status.value, job.home_machine.value, job.last_on.value)
            if job_dir in opt_existing:
                opt_updates.append((*row, opt_existing[job_dir]))
            else:
                opt_inserts.append(row)
        if opt_inserts:
        ...
        # Re-fetch after inserts so dos/wav can resolve opt_id for new opt_jobs
        if opt_inserts:
            opt_existing = {
                row[1]: row[0]
        ...
        # Batch dos_jobs
        dos_existing: Dict[int, int] = {
            row[1]: row[0] for row in self.db.execute("SELECT rowid, opt_id FROM dos_jobs")
        }
        dos_inserts = []
        dos_updates = []
        ...
        # Batch wav_jobs
        wav_existing: Dict[int, int] = {
            row[1]: row[0] for row in self.db.execute("SELECT rowid, opt_id FROM wav_jobs")
        }
        wav_inserts = []
        wav_updates = []
        for job_dir, job in wav_jobs.items():
            ...
            if opt_id in wav_existing:
                wav_updates.append(
                    (
                        job.wav_status.value,
                        job.wav_last_on.value,
                        wav_existing[opt_id],
                    )
                )
            else:
