    ...
    try:
        if machine is Machine.FRI or machine is Machine.HALIFAX:  # fri-halifax
            ...
            while sub_queue_index < num_to_sub_there:
                ...
                if ssh_config.config != "NoSSH":
                    ssh_config.config.ssh.run("cd " + shlex.quote(new_loc) + " && sbatch " + shlex.quote(other_subfile))
            ...
            for i in range(0, 3):
                for _ in range(0, num_will_sub[i]):
                    ...
                    if i + 2 == machine:
                        ...
                        import automagician.update_job as update_job_module
                        update_job_module.switch_subfile(
                            job_dir,
...
def add_to_insta_submit(job_dir: str, machine: str, database: "automagician.database.Database") -> None:
